...
def test_StSoQoSVer004(qos_sched_hooks):
    ...
    for sched,input in zip([sched_0,sched_1,sched_2,sched_3],
        ...
        qos_api.create_qos_json(dut=vars.D1, block_name="SCHEDULER", sub_block=sched,
                    dict_input=input)
    ...
    for queue,input in zip(["5-7","4","3","0-2"],
        ...
        qos_api.create_qos_json(dut=vars.D1, block_name="QUEUE", sub_block=d1_p1 + "|" + queue,
                    dict_input=input)
...
def test_StSoQoSVer008(qos_sched_hooks):
    ...
    qos_api.create_qos_json(dut=vars.D1, block_name=wred_profile, sub_block=wred_obj,
                    dict_input=wred_input)
    st.log("apply WRED config to queue")
    qos_api.create_qos_json(dut=vars.D1, block_name="QUEUE", sub_block=d1_p1 + "|" + "3",
...
def test_StSoQoSVer003(qos_sched_hooks):
    ...
    qos_api.create_qos_json(dut=vars.D1, block_name=port_qos_map, sub_block=d2_d1_p1,
                    dict_input=pfc_bind)
