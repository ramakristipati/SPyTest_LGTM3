...
def ospf_initialize_variables():
    ...
    data.ospf_mib = '1.3.6.1.2.1.14'
    data.bgp_mib = '1.3.6.1.2.1.15'
    data.mgmt_int = 'eth0'
...
@pytest.mark.snmp_ospf_bgp
@pytest.mark.ospf_regression
def test_ft_snmp_ospf_bgp_mib():
    """
    Author : Prasad Darnasi<prasad.darnasi@broadcom.com>
    Verify OSPF and BGP SNMP mib objects
    """
    global ipaddress
    ipaddress_list = basicapi.get_ifconfig_inet(vars.D1, data.mgmt_int)
    ...
    st.banner('Verifying SNMP object for BGP and OSPF')

    snmp_output1 = snmp_obj.poll_for_snmp_walk(vars.D1, 15, 1, ipaddress=ipaddress, oid=data.ospf_mib,
                                               community_name=data.ro_community)
    if not snmp_output1:
       st.report_tc_fail("ft_snmp_ospf_mib", "get_snmp_output_fail", "OSPF-MIB")
    else:
       st.report_tc_pass("ft_snmp_ospf_mib", "test_case_passed", "OSPF-MIB")

    snmp_output2 = snmp_obj.poll_for_snmp_walk(vars.D1, 15, 1, ipaddress=ipaddress, oid=data.bgp_mib,
                                                community_name=data.ro_community)
    if not snmp_output2:
        st.report_tc_fail("ft_snmp_bgp_mib", "get_snmp_output_fail", "BGP-MIB")
    else:
        st.report_tc_pass("ft_snmp_bgp_mib", "test_case_passed", "BGP-MIB")

    if not (snmp_output1 and snmp_output2):
        st.report_fail('test_case_failed')
    else:
        st.report_pass('test_case_passed')

    st.banner('snmp config to defaults')
...
def test_ospf_traffic_verify():
    ...
    for dut, ip_addr in zip([vars.D1, vars.D1, vars.D2], ["201.1.1.0/24", "201.1.5.0/24", "201.1.1.0/24"]):
