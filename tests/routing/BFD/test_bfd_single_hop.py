...
def verify_FtOpSoRoBfdFn055_52_56_57_21(vrfname='default'):
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,peer=dut3_ip_list[0:2]+dut3_ipv6_list[0:2],status=['up']*4,
                                 ...
                                 tx_interval = [['150', '150'], ['300','300']] * 2,retry_count=5,delay=1, ping_verify=True)
    if result is False:
...
def test_FtOpSoRoBfdFn011_20(bfd_fixture_010):
    ...
    for vrfname in ['default',user_vrf_name]:
        ...
        ###########################################################################################
        result = retry_api(bfd.verify_bfd_peer,dut1, vrf_name= vrfname, peer=[dut3_ip_list[0],dut3_ipv6_list[0]],interface=[access_vlan_name]*2,status=['up']*2,retry_count=3,delay=1, ping_verify=True)
        if result is False:
            st.report_fail('bfd_peer_params', dut1)
...
def test_FtOpSoRoBfdFn009(bfd_fixture_012):
    ...
    for vrfname in ['default',user_vrf_name]:
        ...
        result = retry_api(bfd.verify_bfd_peer,dut1, vrf_name=vrfname, peer=[dut3_ip_list[1],dut3_ipv6_list[1]],echo_tx_interval=[['50','50'],['50','50']],status=['up','up'],retry_count=5,delay=1, ping_verify=True)
        if result is False:
...
def test_FtOpSoRoBfdFn036_37_38_40_25(bfd_fixture_013):
    ...
    result = retry_api(bfd.verify_bfd_peer, dut1, peer=[dut3_ip_list[0], dut3_ipv6_list[0]], status=['up'] * 2,
                       retry_count=3, delay=3, ping_verify=True)
    if result is False:
    ...
    if result is False:
        ...
        st.error(err); err_list.append(err); tc_result = False
    for peer,intf in zip([dut3_ip_list[0],dut3_ipv6_list[0]],[access_vlan_name]*2):
...
def test_FtOpSoRoBfdFn089(bfd_fixture_017):
    ...
    if l2_switch == 'yes':
        ...
        result = bfd.verify_bfd_peer(dut1, peer=[dut3_ip_list[0], dut3_ipv6_list[0]], tx_interval=[['10', '1000'], ['10', '1000']],
                                     rx_interval=[['10', '1000'], ['10', '1000']], ping_verify=True)
        if result is False:
...
def test_FtOpSoRoBfdFn035_26(bfd_fixture_018):
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,peer=dut3_ip_list[0:2]+dut3_ipv6_list[0:2],status=['up']*4,retry_count=2,delay=1, ping_verify=True)
    if result is False:
...
def test_FtOpSoRoBfdFn001_60_61(bfd_fixture_019):
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1, peer=[dut3_ip_list[0],dut3_ipv6_list[0]],interface=[access_vlan_name]*2,status=['up']*2, ping_verify=True,
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,vrf_name=user_vrf_name, peer=[dut3_ip_list[0],dut3_ipv6_list[0]],interface=[access_vlan_name_vrf]*2,status=['up']*2, ping_verify=True,
                                 rx_interval=[['210','300'],['320','300']],tx_interval=[['200','300'],['290','300']],echo_tx_interval=[['100','50'],['120','50']],retry_count=2,delay=1)
    if result is False:
    ...
    if not st.poll_wait(basic.get_system_status, 60, vars.D1):
        st.error('System status is not up after doing BGP docker start.')
        st.report_fail('bfd_fail_reason', 'System is not ready after BGP docker restart')
    st.wait(5)
    reboot_api.verify_warm_restart(dut1, mode='config')
...
def test_FtOpSoRoBfdFn090_92_93(bfd_fixture_021):
    ...
    ###########################################################################################
    result = retry_api(bfd.verify_bfd_peer,dut1, peer=[dut3_ip_list[2],dut3_ipv6_list[2]], interface=[trunk_vlan_name[0]]*2, status=['up'] * 2, ping_verify=True)
    if result is False:
...
def test_FtOpSoRoBfdFn019(bfd_fixture_022):
    ...
    for vrfname in ['default', user_vrf_name]:
        ...
        result = retry_api(bfd.verify_bfd_peer,dut1, vrf_name= vrfname, peer=dut2_link_local,local_addr=dut1_link_local,interface=D1_ports[1],status='up',retry_count=2,delay=1, ping_verify=True)
        if result is False:
...
def test_FtOpSoRoBfdFn094(bfd_fixture_024):
    ...
    ###########################################################################################
    result= bfd.verify_bfd_peer(dut1,peer=dut3_ip_list[2],interface=trunk_vlan_name[0],status='up', ping_verify=True)
    if result is False:
...
def verify_bfd_func_001(mode='access'):
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,peer=bfd_nbrs_dut1,interface=intf_list,rx_interval=[['300','300']]*len(bfd_nbrs_dut1), ping_verify=True,
    ...
    for peer in bfd_nbrs_dut1:
        ...
        if result:
            ...
            ret_val = False
    if ret_val is False: basic.get_techsupport(filename='FtOpSoRoBfdFn003_12_10_15_51_53_54')
...
def verify_singlehop_bfd_vrf_functionality():
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,vrf_name = user_vrf_name, peer=bfd_nbrs_dut1,interface=intf_list,rx_interval=[['300','300']]*len(bfd_nbrs_dut1),ping_verify=True,
                                 status=['up']*len(bfd_nbrs_dut1),tx_interval=[['300','300']]*len(bfd_nbrs_dut1),multiplier=[['3','3']]*len(bfd_nbrs_dut1))
    if result is False:
        st.error("FAILED : BFD session parameters mismatch ")
        ret_val = False
    ...
    if ret_val is False: basic.get_techsupport(filename='singlehop_bfd_vrf_functionality')
    ###########################################################################################
    hdrMsg("Step T4: Verify BFD is disabled for other BGP neighbors %s"% non_bfd_nbrs_dut1)
    ###########################################################################################
...
def test_bfd_vrf_save_and_reload(bfd_fixture_019):
    ...
    result = retry_api(bfd.verify_bfd_peer, dut1, peer=[dut3_ip_list[0], dut3_ipv6_list[0]],
                       ...
                       echo_tx_interval=[['100', '50'], ['120', '50']], retry_count=2, delay=1, ping_verify=True)
    if result is False:
        st.report_fail('bfd_fail_reason', "BFD parameters are incorrect for one or more BFD ipv4 peers")
