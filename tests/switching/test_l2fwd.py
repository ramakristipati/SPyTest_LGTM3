...
import utilities.utils as r_utils
...
def create_mac_move_streams(pkt_rate=100):
    ...
    ### In Spirent, stream config is port based.
    ### If few of streams within same port is configured burst, it will change other streams' mode as well
...
def verify_mac_table(dut_list,expect_mac_list,vlan=None,port=None,type=None,mac_search=None,comp_flag='equal'):
    ...
    if not retry_parallel(show_verify_mac_table, dict_list=dict_list,dut_list=dut_list, retry_count=3, delay=2):
        ...
        print_log("DEBUG: Vlan info", "MED")
        utils.exec_all(True, [[vlan_obj.show_vlan_brief,dut] for dut in dut_list])
...
def test_l2fwd_lag(lag_function_fixture):
    ...
    for po_key in po_list:
        ...
        D2D1_po_members = data.portchannel[po_key]["dut2_members"]
        unconfigure_portchannel(dut1, dut2, po_name, vlan, D1D2_po_members, D2D1_po_members)
        min_link = int(len(D1D2_po_members)/2) + 1
...
def test_static_mac():
    ...
    for d2 in data.my_dut_list[1:]:
        ...
        po_key = data.po_metadata[(d1, 'trunk')]['base_id']
        trunk_lag   = data.portchannel[po_key]['name']
        utils.exec_all(True, [[vlan_obj.config_vlan_range_members, d1, vlan_phyT, all_links[base_lagT_link][0],'del'], \
                              [vlan_obj.config_vlan_range_members, d2, vlan_phyT, all_links[base_lagT_link][2],'del']])
...
def cleanup_stp_mac_move():
    yield
    global stp_deconf_flag, port_deconf_flag, port_enable_flag
...
def test_stp_mac_move(cleanup_stp_mac_move):
    ...
    ### -- clear MAC - to clear learned MACs from prev TCs
    ...
    if st.get_ui_type(d1) in ['rest-put','rest-patch']:
        print_log("Spanning-tree output in klish mode for debugging",'MED')
        for vlan in vlans:
            utils.exec_all(True, [[pvst_obj.show_stp_vlan, dut, vlan, 'klish'] for dut in [d1, d2]])
        verify_po_state(po_key_list=[lagT_po_key])
    ...
    if mac_count_fail > 0:
        st.generate_tech_support(dut=None, name='stp_reconvergence_mac_fail')
    ...
    print_log("Start uni-direction traffic From D1 to D2 and Verify L2 Loop is present in vlan 1 & vlan 102",'MED')
    ...
    print_log("Verify flooding in access vlan ",'MED')
    if not retry_func(verify_flooding_on_dut, dut=d2, portList=[d2_trunk_port], threshold=expect_flood_packets,
                      delay=3, retry_count=3):
        print_log('FAIL:Traffic flooding , expect_tx_pkt > {} is not seen in Dut:{}, Port:{}'.format(
            expect_flood_packets, d2, d2_trunk_port), 'ERROR')
        final_result = False
        flooding_fail += 1
    else:
        print_log('PASS:Traffic flooding expect_tx_pkt > {} is seen in Dut:{}, Port:{}'.format(
            expect_flood_packets, d2, d2_trunk_port), 'MED')

    print_log("Verify flooding in access LAG vlan", 'MED')
    if not retry_func(verify_flooding_on_dut, dut=d2, portList=[lagT_po_name], threshold=expect_flood_packets,
                      delay=3, retry_count=3):
        print_log('FAIL:Traffic flooding, expect_tx_pkt > {} is not seen in Dut:{}, Port:{}'.format(
            expect_flood_packets, d2, lagT_po_name), 'ERROR')
        final_result = False
        flooding_fail += 1
    else:
        print_log('PASS:Traffic flooding expect_tx_pkt > {} is seen in Dut:{}, Port:{}'.format(
            expect_flood_packets, d2, lagT_po_name), 'MED')

    ### Verify Dampening Outputs when loop present
    print_log("Verify Mac Move Dampening when L2 loop present", 'HIGH')
    ...
    results = pll.exec_parallel(pll_exec, [data.ver_dut_list[0]], mac_obj.verify_mac_dampening_disabled_ports,
                                              [dict1])[0]
    if True in results:
        print_log("Mac Move Dampening disabled ports FAILED, Expected ports not seen in disabled list", 'MED')
        mac_dampen_fail += 1
        ### Check input rate = 3 * o/p
        ### check on reverse SA mac stream -- continuous traffic
        ### increase 1000 pkt/sec to 3000 and see o/p rate of loop ports
        ### Same traffic action to be done in 2nd loop
        #import pdb;
        #pdb.set_trace()
    else:
    ...
    ###Collecting techsupport takes about 4 min. Hence collect it after dampening check.
    if flooding_fail > 0:
    ...
    print_log("Verify flooding is not seen in access vlan", 'MED')
    if not retry_func(verify_flooding_on_dut, dut=d2, portList=[d2_trunk_port], threshold=data.counters_threshold,
                      comp_flag=False, delay=3, retry_count=3):
        print_log('PASS:Traffic flooding is not seen in Dut:{}, Port:{}, expect_tx_pkt < {}'.format(
            d2, d2_trunk_port,data.counters_threshold), 'MED')
    else:
        print_log('FAIL:Traffic flooding is still seen in Dut:{}, Port:{}, expect_tx_pkt < {}'.format(
            d2, d2_trunk_port, data.counters_threshold), 'ERROR')
        expect_loop_break = 0
        final_result = False
        flooding_fail += 1

    print_log("Verify flooding is not seen in access LAG vlan", 'MED')
    if not retry_func(verify_flooding_on_dut, dut=d2, portList=[lagT_po_name], threshold=data.counters_threshold,
                      comp_flag=False,delay=3,retry_count=3):
        print_log('PASS:Traffic flooding is not seen in Dut:{}, Port:{}, expect_tx_pkt < {}'.format(
            d2, lagT_po_name, data.counters_threshold), 'MED')
    else:
        print_log('FAIL:Traffic flooding is still seen in Dut:{}, Port:{}, expect_tx_pkt < {}'.format(
            d2, lagT_po_name, data.counters_threshold), 'ERROR')
        expect_loop_break = 0
        final_result = False
        flooding_fail += 1

    if expect_loop_break:
    ...
    ## In Mac Move Dampening If loop seen within 10 sec of a port re-enabled for MAC learning, it will be again disabled for 2*180 sec
    ## To avoid this sleep extra 10 sec
    current_time = time.time()
    time_elapsed = current_time - start_time
    st.wait(190-time_elapsed, "MAC Dampening: Wait for remaining time of 180+10 sec for port to get re-enabled for MAC Learning")

    print_log("Verify Dampening disabled ports are re-enabled with default Mac Dampening threshold count & interval",'MED')
    verify_mac_dampening_disabled_ports(0)

    ###Enable back the ports and Revert port-vlan assosciation,
    st.banner("Enable back the ports and start traffic.\n Remove port-vlan configs causing the loop to check loop resolved")
    utils.exec_all(True, [[intf_obj.interface_operation, dut, dut_port_dict[dut], 'startup'] for dut in
                          dut_port_dict.keys()])
    port_deconf_flag = 0
    ### Start traffic to create traffic loop and modify port-vlan assosciation to see loop is broke.
    ## Intention is to check, vlan-port config changes can break loop when l2 loop is in system
    start_stop_traffic(src_stream_list=data.base_src_streams, dest_stream_list=data.base_dst_streams,duration=traffic_time)
    ### UnConfigure base_access_vlan from trunk phyPort
    api_list = []
    api_list.append([vlan_obj.delete_vlan_member, d1, data.base_access_vlan, d1_trunk_port, True])
    ...
    st.wait(mac_dampening_timer+10,"MAC Dampening: Wait for {} seconds for MAC learning to get re-enabled".format(mac_dampening_timer))
    verify_mac_dampening_disabled_ports(0)
...
def test_FtOpSoSwL2FwdFnCeta31752(lag_function_fixture):
    ...
    tx1=port_obj.get_interface_counters_all(data.my_dut_list[0], vars.D1T1P2)
    tx1=int(tx1[0]['rx_pps'].split(".")[0])
    st.log("########## DUT1, RX_PPS value is : {} ##########".format(tx1))
    po_key_t = data.po_metadata[(data.my_dut_list[0], 'trunk')]['base_id']
    rx1=port_obj.get_interface_counters_all(data.my_dut_list[1], data.portchannel[po_key_t]["dut2_members"][0])
    rx1=int(rx1[0]['rx_pps'].split(".")[0])
    st.log("########## DUT2, RX_PPS value for LAG member port1 is : {} ##########".format(rx1))
    rx2=port_obj.get_interface_counters_all(data.my_dut_list[1], data.portchannel[po_key_t]["dut2_members"][1])
    rx2 = int(rx2[0]['rx_pps'].split(".")[0])
    st.log("########## DUT2, RX_PPS value for LAG member port2 is : {} ##########".format(rx2))
    total_rx = rx1 + rx2
...
def verify_mac_dampening_disabled_ports(expect=1,msg='dampening timer expiry',retry_count=10, delay=2):
    ret_val = True
    if expect == 0:
        dict1 = {'port_list': ['None']}
        dict2 = {'port_list': ['None']}
        result = r_utils.retry_parallel(mac_obj.verify_mac_dampening_disabled_ports, dut_list=data.ver_dut_list,
                                                  dict_list=[dict1, dict2],retry_count=retry_count, delay=delay)
        if not result:
            print_log("Mac Move Dampening disabled ports FAILED, Expect no ports to be disabled", 'MED')
            ret_val = False
            fail_msg = "Mac Move Dampening disabled ports seen after {}, FAILED".format(msg)
            st.report_fail("test_case_failure_message", fail_msg)
        else:
            print_log("Mac Move Dampening disabled ports PASSED", 'MED')
    else:
        ...
        for i in range(retry_count):
            st.log("Attempt %s of %s" % ((i + 1), retry_count))
            result_dict = utils.exec_foreach(pll_exec, data.ver_dut_list, mac_obj.verify_mac_dampening_disabled_ports,
                                             return_output=1)[0]
            ...
            if d1_result and d2_result:
                ...
                if retry_count != (i + 1):
                    st.wait(delay,"waiting for %s seconds before retry" % delay)
...
def verify_mac_learning(expect='disable',msg='dampening timer expiry'):
    port_disable_fail = 0
    d1 = data.ver_dut_list[0]
    lagT_po_key = data.po_metadata[(d1, 'trunk')]['base_id']
    lagT_po_name = data.portchannel[lagT_po_key]['name']
    ...
    duration = 2 if expect == 'enable' else 1

    if expect == 'disable':
        if vars.D1T1P2 in data.disabled_port_list['d1']:
            if not verify_mac_table([d1], [0], port=vars.D1T1P2):
                port_disable_fail += 1
            if vars.D1D2P1 in data.disabled_port_list['d1'] and not verify_mac_table([d1], [0], port=vars.D1D2P1):
                port_disable_fail += 1
            if vars.D1D2P2 in data.disabled_port_list['d1'] and not verify_mac_table([d1], [0], port=vars.D1D2P2):
                port_disable_fail += 1
            if lagT_po_name in data.disabled_port_list['d1'] and not verify_mac_table([d1], [0], port=lagT_po_name):
                port_disable_fail += 1
            if lagA_po_name in data.disabled_port_list['d1'] and not verify_mac_table([d1], [0], port=lagA_po_name):
                port_disable_fail += 1
        else:
            if not verify_mac_table([d1], [0], port=vars.D1D2P1):
                port_disable_fail += 1
            if not verify_mac_table([d1], [0], port=vars.D1D2P2):
                port_disable_fail += 1
            if not verify_mac_table([d1], [0], port=lagT_po_name):
                port_disable_fail += 1
            if not verify_mac_table([d1], [0], port=lagA_po_name):
                port_disable_fail += 1
        ...
        if not verify_mac_table(data.ver_dut_list, data.dut_mac_count_list, mac_search='00:'):
            ...
            fail_msg = "Mac Re-learning not seen on disabled ports after {}, FAILED".format(msg)
            st.report_fail("test_case_failure_message", fail_msg)
        else:
            print_log("PASS: Mac Re-learning seen on disabled ports after {}".format(msg), 'MED')

def test_l2fwd_mac_move_dampening(cleanup_mac_move_dampening):
    final_result = True
    traffic_forward_fail = 0
    tc_list = ['FtOpSoSwMacDampeningNonDefaultThreshold', 'FtOpSoSwMacDampeningDisabledPortMacRelearn',
               'FtOpSoSwMacDampeningTimerProgression']
    print_log("START of TC ==> test_l2fwd_mac_move_dampening\n TCs:<{}>".format(tc_list), "HIGH")

    ### Create stream on DutN with SA same as Dut1 SA -- 8 pkts/sec for 5 sec
    create_mac_move_streams(pkt_rate=8)
    ...
    st.banner("Configure Higher Mac Move Dampening threshold 100 and verify ports not disabled")
    ...
    dict2 = {'port_list': ['None']}
    results = \
    pll.exec_parallel(pll_exec, data.ver_dut_list, mac_obj.verify_mac_dampening_disabled_ports, [dict1, dict2])[0]
    if False in results:
        print_log("Mac Move Dampening disabled ports FAILED, Expect no ports to be disabled", 'MED')
    ...
    ### Send MacMov traffic and verify Dampening is in effect.\n Also send another traffic and Mac learning disabled for ports.
    st.banner("Send MacMov traffic and verify Dampening is in effect for 180 sec.")
    ...
    if log.get_logging_count(data.ver_dut_list[0], severity="WARNING",
                             filter_list=["disabled for mac learning for 180 seconds"]):
        print_log("PASS: Syslog seen - Port Mac Learning disabled for 180 sec", 'MED')
    else:
        print_log("FAIL: Syslog not seen - Port Mac Learning disabled for 180 sec", 'MED')
        fail_msg = "Mac Move Dampening syslog verification FAILED"
        st.report_fail("test_case_failure_message", fail_msg)

    print_log("Clear MAC and verify Mac learning disabled for ports.",'MED')
    utils.exec_all(pll_exec, [[mac_obj.clear_mac, dut] for dut in data.my_dut_list])
    verify_mac_learning()
    ...
    st.banner("Verify Mac Dampening timer increased to 360 sec")
    st.wait(5, 'Continuous stream flowing, Wait 5 sec for mac move packets to reach threshold')

    if log.get_logging_count(data.ver_dut_list[0],severity="NOTICE", filter_list=["re-enabled for mac learning"]):
        print_log("PASS: Syslog seen - Port Mac Learning re-enabled",'MED')
    else:
        print_log("FAIL: Syslog not seen - Port Mac Learning re-enabled", 'MED')
        fail_msg = "Mac Move Dampening syslog verification FAILED"
        st.report_fail("test_case_failure_message", fail_msg)
    ...
    if log.get_logging_count(data.ver_dut_list[0], severity="WARNING",
                             filter_list=["disabled for mac learning for 360 seconds"]):
        print_log("PASS: Syslog seen - Port Mac Learning disabled for 360 sec", 'MED')
    else:
        print_log("FAIL: Syslog not seen - Port Mac Learning disabled for 360 sec", 'MED')
        if log.get_logging_count(data.ver_dut_list[0], severity="WARNING",
                              filter_list=["disabled for mac learning for"]) < 2:
            fail_msg = "Mac Move Dampening 2nd time, syslog verification FAILED"
            st.report_fail("test_case_failure_message", fail_msg)
        else:
            ### If same port is not disabled again or
            # the disabling didn't happen within 10 sec of re-enabling,
            # Disable timer will be again 180 sec -- expected -- check 180 sec is hit again.
            #If Yes, report TC unsupported Else, report TC fail
            print_log("Unable to verify TC: < FtOpSoSwMacDampeningTimerProgression > as scenario is not triggered\nSkipping TC.", 'MED')
            st.report_unsupported("test_execution_skipped","TC: < FtOpSoSwMacDampeningTimerProgression > Skipped as scenario is not triggered")

    ### Wait for mac age time (100 sec) and see macs cleared-- verfy new mac not learned
    st.wait(2* mac_age_time,'Wait for twice mac age time {} and verify no new MAC learning on disabled ports'.format(mac_age_time))
    utils.exec_all(True, [[mac_obj.get_mac, dut] for dut in data.ver_dut_list])
    utils.exec_all(True, [[mac_obj.clear_mac, dut] for dut in data.ver_dut_list])
    verify_mac_dampening_disabled_ports()
    verify_mac_learning()
    ...
    ### Send MacMov traffic within 10 sec of re-enabling ports and verify dampening timer increases as 180sec - > 360 sec -> 720 sec
    st.banner("Mac Move will be detected 3rd time\nVerify Mac Dampening timer increased to 720 sec")
    st.wait(5, 'Continuous stream flowing, Wait 5 sec for mac move packets to reach threshold')

    if log.get_logging_count(data.ver_dut_list[0],severity="NOTICE", filter_list=["re-enabled for mac learning"]) > 1:
        print_log("PASS: Syslog seen - Port Mac Learning re-enabled",'MED')
    else:
        print_log("FAIL: Syslog not seen - Port Mac Learning re-enabled", 'MED')
        fail_msg = "Mac Move Dampening syslog verification FAILED"
    ...
    verify_mac_dampening_disabled_ports()
    if log.get_logging_count(data.ver_dut_list[0], severity="WARNING",
                             filter_list=["disabled for mac learning for 720 seconds"]):
        print_log("PASS: Syslog seen - Port Mac Learning disabled for 720 sec", 'MED')

        ### To validate 720 sec dampening timer, wait for 400 sec and verify dampening timer is >360 sec
        ## This is done to reduce script execution time
        st.banner("To validate 720 sec dampening timer, wait for 400 sec and verify dampening timer is >360 sec")
        current_time = time.time()
        time_elapsed = current_time - start_time
        st.wait(400 - time_elapsed, "MAC Move Dampening: Wait for 400 Sec and verify dampening still in effect")
        utils.exec_all(True, [[mac_obj.clear_mac, dut] for dut in data.ver_dut_list])
        verify_mac_dampening_disabled_ports()
        verify_mac_learning()
    else:
        ### If same port is not disabled again or
        # the disabling didn't happen within 10 sec of re-enabling,
        # Disable timer will be 180 sec -- expected -- check 180 sec is hit again.
        print_log("FAIL: Syslog not seen - Port Mac Learning disabled for 720 sec", 'MED')
        if log.get_logging_count(data.ver_dut_list[0], severity="WARNING",
                              filter_list=["disabled for mac learning for"]) < 3:
            print_log("FAIL: Syslog not seen - Port Mac Learning disabled for 3rd time", 'MED')
            fail_msg = "Mac Move Dampening 3rd time syslog verification FAILED"
            st.report_fail("test_case_failure_message", fail_msg)
...
def add_rem_port_vlan_configs(config='add'):
    ...
    trunk_vlan_range = str(data.base_trunk_vlan)+' '+str(data.base_trunk_vlan+data.trunk_vlan_count-1)
    lagT_vlan_range = str(data.base_lagT_vlan)+' '+str(data.base_lagT_vlan+data.lagT_vlan_count-1)
    vlan_range = str(data.base_vlan) + " " + str(data.base_lagA_vlan + data.lagA_count - 1)
    ...
    if config == 'add':
        def func(dut):
            ...
            if f1:
                ### UnConfigure vlans from D1 tgen port and links between duts
                vlan_obj.config_vlan_range_members(d1, [vlan_range, data.tc_specific_vlan_range], vars.D1T1P2)
                vlan_obj.add_vlan_member(d1, data.base_access_vlan, d1_access_port)
                vlan_obj.config_vlan_range_members(d1, trunk_vlan_range, d1_trunk_port)
                vlan_obj.config_vlan_range_members(d1, lagT_vlan_range, lagT_po_name)
                vlan_obj.add_vlan_member(d1, data.base_lagA_vlan, lagA_po_name)

            if f2:
                ### UnConfigure vlans from D2 tgen port and links between duts
                vlan_obj.config_vlan_range_members(d2, [vlan_range, data.tc_specific_vlan_range], vars.D2T1P1)
                vlan_obj.add_vlan_member(d2, data.base_access_vlan, d2_access_port)
                vlan_obj.config_vlan_range_members(d2, trunk_vlan_range, d2_trunk_port)
                vlan_obj.config_vlan_range_members(d2, lagT_vlan_range, lagT_po_name)
                vlan_obj.add_vlan_member(d2, data.base_lagA_vlan, lagA_po_name)

        st.exec_each([d1, d2], func)
        ...
        def func(dut):
            ...
            if f1:
                ### UnConfigure vlans from D1 tgen port and links between duts
                vlan_obj.config_vlan_range_members(d1, [vlan_range,data.tc_specific_vlan_range], vars.D1T1P2, 'del')
                vlan_obj.delete_vlan_member(d1, data.base_access_vlan, d1_access_port)
                vlan_obj.config_vlan_range_members(d1, trunk_vlan_range, d1_trunk_port, 'del')
                vlan_obj.config_vlan_range_members(d1, lagT_vlan_range, lagT_po_name, 'del')
                vlan_obj.delete_vlan_member(d1, data.base_lagA_vlan, lagA_po_name)

            if f2:
                ### UnConfigure vlans from D2 tgen port and links between duts
                vlan_obj.config_vlan_range_members(d2, [vlan_range,data.tc_specific_vlan_range], vars.D2T1P1, 'del')
                vlan_obj.delete_vlan_member(d2, data.base_access_vlan, d2_access_port)
                vlan_obj.config_vlan_range_members(d2, trunk_vlan_range, d2_trunk_port, 'del')
                vlan_obj.config_vlan_range_members(d2, lagT_vlan_range, lagT_po_name, 'del')
                vlan_obj.delete_vlan_member(d2, data.base_lagA_vlan, lagA_po_name)
...
def test_reload_with_mac_dampening(cleanup_reload_with_mac_dampening):
    ...
    print_log("START of TC ==> test_reload_with_mac_dampening\n TCs:<{}>".format(tc_list), "HIGH")

    ### Create traffic stream on DutN with SA same as Dut1 SA -- 10 pkts/sec for 5 sec
    create_mac_move_streams(pkt_rate=500)
    utils.exec_all(pll_exec, [[mac_obj.clear_mac, dut] for dut in data.my_dut_list])
    utils.exec_all(pll_exec, [[mac_obj.clear_mac_dampening, dut] for dut in data.my_dut_list])
    ...
    verify_mac_dampening_disabled_ports(expect=0, msg='port vlan unconfig')
    print_log("Mac Move Dampening: clear disabled ports after port vlan unconfig PASSED", 'HIGH')
    ...
    if data.tgen_type == "stc":
        ...
        st.wait(4,'Continuous stream started, Wait 4 sec for mac move packets to reach threshold')
    verify_mac_dampening_disabled_ports(msg="port vlan reconfig")

    boot_obj.config_save_reload(data.ver_dut_list)

    results = utils.exec_foreach(pll_exec, data.my_dut_list, mac_obj.verify_mac_dampening_threshold, count=30)[0]
    if False in results:
        print_log("Mac Move Dampening threshold config FAILED after reboot", 'MED')
        fail_msg = "Mac Move Dampening Threshold config failed after reboot"
        st.report_fail("test_case_failure_message", fail_msg)
    else:
        print_log("Mac Move Dampening threshold config PASSED", 'MED')
    ...
    verify_mac_dampening_disabled_ports()
    start_time = time.time()
    ## Clear mac to clear already learned MACs,and verify MAC not learned in next step.
    utils.exec_all(pll_exec, [[mac_obj.clear_mac, dut] for dut in data.my_dut_list])
    st.wait(1,"Wait for 1 sec, and Verify MAC is not learned on disabled ports with ongoing traffic")
    verify_mac_learning()
    ...
    verify_mac_learning(expect='enable')
    print_log("Mac Move Dampening after reboot PASSED", 'HIGH')
